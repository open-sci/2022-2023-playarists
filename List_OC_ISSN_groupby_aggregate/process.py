...
def process_meta_csv(chunk, erih_plus_df):
    ...
    #merged_data = merged_data.groupby('EP_ID').agg({'OC_ISSN': list, 'OC_OMID': 'first', 'EP_ISSN': lambda x: list(set(x))}).reset_index()
    merged_data = merged_data.groupby('EP_ID').agg({'OC_ISSN': list, 'OC_OMID': 'first'}).reset_index()


    return merged_data


def process_meta_csv_saved(chunk, erih_plus_df):
    meta_data = chunk
    meta_data['venue'] = meta_data['venue'].astype(str)
    meta_data = meta_data.dropna(subset=['venue'])  
    meta_data['venue'] = meta_data['venue'].str.strip()
    ...
    #meta_data['OC_OMID'] = meta_data['id'].str.extract(r'(meta:([^\s]*))')[1]
...
def process_file(input_file, erih_plus_df):
    ...
    dataf = dataf.groupby('EP_ID').agg({'OC_ISSN': list, 'OC_OMID': 'first'}).reset_index()
