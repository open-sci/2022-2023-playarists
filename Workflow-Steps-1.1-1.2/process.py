...
def process_meta_csv(chunk, erih_plus_dict):
    ...
    meta_data = meta_data.groupby('issn').agg({'id': lambda x: len(x.tolist()), 'issn': lambda x: x.iloc[0]})
    ...
    for idx, row in meta_data["issn"].items():
        for el in row[1:-1].split(", "):
            ...
            if el in lists:
                idx_set.add(idx)
                meta_data.at[idx, 'erih_id'] = erih_plus_dict[el]
    idx_list = list(idx_set)
    result = meta_data.loc[idx_list]
    
    result = result[['id', 'issn', 'erih_id']]    
    return result
